...
if Facter.value(:kernel) == 'windows'
  def powershell_xml_get_version
    ...
    result = nil
    # version.xml can be in Unicode
    file_path = 'version.xml'
    if File.exists?(file_path)
      begin
        script = "param($file_path) ([xml](get-content -path \"${file_path}\" )).Info.Product.version"
        ...
        command = "#{powershell_exec} #{powershell_flags} -command \" & { #{script} }\" \"#{file_path}\""
