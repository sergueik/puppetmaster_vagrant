...
powershell.exe -executionpolicy remotesighed -noprofile -command " & { $input = $args[0] ; $date = Get-Date($input); $seconds = [Math]::Floor([decimal](Get-Date($date) -uformat '%%s')); write-output $seconds} " "'%From_date%'"
GOTO :EOF
:NOARG
set F=%temp%\a.txt
date /t > %F%
...
REM  powershell.exe -executionpolicy remotesighed -noprofile -command " & { $input = $args[0] ; $date =  Get-Date($input); write-output ($date);} "  "'%%.'"
  powershell.exe -executionpolicy remotesighed -noprofile -command " & { $input = $args[0] ; $date = Get-Date($input); $seconds = [Math]::Floor([decimal](Get-Date($date) -uformat '%%s')); write-output $seconds} " "'%%.'"
)
del /q %F%
